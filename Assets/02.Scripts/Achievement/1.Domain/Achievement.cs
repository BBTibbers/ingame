...
public class Achievement
{
    ...
    public event Action<Achievement> OnBecameClaimable;
    ...
    public int CurrentValue
    {
        ...
        set
        {
            bool wasClaimable = CanClaimReward();
            _currentValue = value;
            bool isClaimable = CanClaimReward();

            // 조건이 false → true로 바뀌는 순간
            if (!wasClaimable && isClaimable)
            {
                OnBecameClaimable?.Invoke(this);
            }
        }
    }
